...
class TestLeakageSensorBase:
    ...
    @staticmethod
    def test_accessors_follow_recorded_leak_state():
        '''
        Test get_leak_severity and is_test_leak read self.leaking, which an
        is_leak() override that reads hardware must record, so one is_leak()
        call per poll gives a consistent view without extra hardware reads
        '''
        class HardwareSensor(LeakageSensorBase):
            ...
            def is_leak(self):
                # Reads "hardware" state and records it, as documented
        ...
        # An injected leak recorded by the override is flagged
        sensor.test_leak = True
        assert sensor.is_test_leak() == True
